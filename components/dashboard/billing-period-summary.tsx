...
export function BillingPeriodSummary() {
  ...
  const getBudgetStatus = () => {
    ...
    const hasCostAlert = alerts.some(
      alert => alert.type === 'cost_limit' && alert.isActive
    )

    if (hasCostAlert) {
      const criticalCostAlert = alerts.some(
        alert =>
          alert.type === 'cost_limit' &&
  ...
  const getTokenStatus = () => {
    ...
    const hasTokenAlert = alerts.some(
      alert => alert.type === 'token_limit' && alert.isActive
    )

    if (hasTokenAlert) {
      const criticalTokenAlert = alerts.some(
        alert =>
          alert.type === 'token_limit' &&
          alert.level === 'critical' &&
          alert.isActive
      )
