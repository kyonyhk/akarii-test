...
interface MessageHistoryGroupedProps {
...
const ITEM_HEIGHT_GROUPED = 60 // Height for grouped messages (compact)
const DATE_SEPARATOR_HEIGHT = 40

export function MessageHistoryGrouped({
  conversationId,
  userId,
  userName = 'Anonymous',
  className,
  ...
  const virtualItems = useMemo(() => {
    ...
    messageGroups.forEach(group => {
      ...
      group.messages.forEach(message => {
        ...
        // Adjust height based on message variant and analysis
        if (message.variant === 'grouped') {
          height = ITEM_HEIGHT_GROUPED
        }

        if (showAnalysis && analysisMode !== 'none') {
          height += 60 // Add space for analysis card
        }
  ...
  const loadMessages = async (cursor?: string) => {
    try {
      ...
      const mockMessages: Message[] = Array.from({ length: 50 }, (_, i) => {
        ...
        return {
          _id: `msg-${cursor || 'initial'}-${i}` as Id<'messages'>,
  ...
  const VirtualItem = ({
    ...
    const message = item.data as ProcessedMessage
    return (
      <div style={style}>
        <MessageBubble
