...
export const runAlertMonitoring = internalAction({
  handler: async ctx => {
    ...
    try {
      ...
      for (const team of teams) {
        try {
          ...
          if (triggeredAlerts && triggeredAlerts.length > 0) {
            console.log(
              `Found ${triggeredAlerts.length} alerts for team ${team.name}`
            )
            ...
            console.log(
              `Processed notifications for team ${team.name}: ${notificationResult.alertsProcessed} alerts`
            )
          }
          ...
          console.error(
            `Error processing alerts for team ${team.name}:`,
            teamError
          )
        }
      }
      ...
      console.log(
        `Alert monitoring completed: ${processedTeams} teams processed, ${totalAlerts} alerts triggered`
      )
...
export const healthCheck = internalAction({
  handler: async ctx => {
    ...
    try {
      ...
      const configCount = await ctx.runQuery(
        'alert_configs:getAlertConfigCount'
      )
      checks.push({
    ...
    try {
      ...
      const testNotification = await ctx.runAction(
      ...
      checks.push({
        ...
        details: testNotification.success
